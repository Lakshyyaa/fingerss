...
function App(props) {
  ...
  const fetchWords = useCallback(() => {
    fetch('https://random-word-api.herokuapp.com/word?number=10')
      ...
      .then(words => setWords(words.join(' ')))
      .catch(error => console.error(error));
  }, []);

  const startTimer = useCallback((countdownTime) => {
    if (countdownTime > 0) {
      setTimerText(countdownTime);
      setTimeout(() => {
        startTimer(countdownTime - 1);
      }, 1000);
    } else {
      countdowndiv.current.style.display = 'none';
      maindiv.current.style.opacity = '1';
  ...
  const handleKeyPress = useCallback((e) => {
    ...
    const index = 0;
    if (isStarted) {
      beginTyping(index, typed);
    }
  }, [isStarted]);

  const beginTyping = useCallback((index, typed) => {
