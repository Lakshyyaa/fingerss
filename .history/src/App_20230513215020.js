...
function App(props) {
  ...
  function beginTyping(i) {
    if (boolstart) {
      const letterSpan = spandiv.current.children[i]
      ...
      if (typed === wordsRef.current[i]) {
        ...
        function beginTyping(i) {
          if (boolstart) {
            if (typed === wordsRef.current[i]) {
              // Update the letter to be green
              spandiv.current.children[i].classList.add('green');

              index++;
              keyStrokes++;
            } else {
              // Update the letter to be red
              spandiv.current.children[i].classList.add('red');
